...
|--- |--- |--- |
| biometrics_minor   | 0.2.14  |  <https://github.com/msk-access/biometrics> |
...
```bash
    > toil-cwl-runner biometrics_minor.cwl example_inputs.json
```

```bash
#Using CWLTOOL
> cwltool --singularity --non-strict /path/to/biometrics_minor.cwl /path/to/example_inputs.json

#Using toil-cwl-runner
> mkdir tool_toil_log
> toil-cwl-runner --singularity --logFile /path/to/tool_toil_log/cwltoil.log  --jobStore /path/to/tool_jobStore --batchSystem lsf --workDir /path/to/tool_toil_log --outdir . --writeLogs /path/to/tool_toil_log --logLevel DEBUG --stats --retryCount 2 --disableCaching --maxLogFileSize 20000000000 /path/to/biometrics_minor.cwl /path/to/example_inputs.json > tool_toil.stdout 2> tool_toil.stderr &
```

### Usage

```bash
> toil-cwl-runner biometrics_minor.cwl -h
usage: biometrics_minor/0.2.14/biometrics_minor.cwl [-h] --input INPUT [--database DATABASE]
                                                    [--minor_threshold MINOR_THRESHOLD] [--prefix PREFIX]
